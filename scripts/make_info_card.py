...
ROWS = [
    ("host",),
    ("kv", "Now", "Software Engineer @ Chatzy.ai"),
    ("kv", "Prev", "SWE Intern @ Chatzy.ai"),
    ("kv", "Also", "DRIVEX — crack measurement from a phone camera"),
    ("kv", "Edu", "B.E. CS, Chandigarh University '26"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "TypeScript, React, Next.js, React Native"),
    ("kv", "Backend", "Node, Python, FastAPI"),
    ("kv", "Data", "PostgreSQL, MongoDB, Redis, Supabase"),
    ("kv", "AI", "Voice agents, RAG, YOLO, SAM"),
    ("kv", "Cloud", "Docker, Vercel, AWS"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Voice AI agents live for 2000+ businesses"),
    ("bul", "Hindi / Hinglish agents · UAE + India clients"),
    ("bul", "Ship solo, end to end"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">vinciarya@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">vinciarya</tspan><tspan fill="{MUTED}">@</tspan>'
